// Унаследуйте новое исключение от исключения, полученного в упражнении 2. Измените блок оператора catch таким образом, чтобы в нем происходила обработка как производного, так и базового исключений. Добавьте 3х уровневый вызов функции
...
class OutOfMemory3 : public OutOfMemory2 { // Производный класс
    public:
        int num3; // Переменная-член
        int getNum3(){ // Метод доступа
            return num3; // Возвращает значение переменной-члена
        }
};
int main()
{
    try // Попытка выполнить код внутри блока try
    {
        ...
        throw OutOfMemory3(); // Генерация исключения
        }
        catch (OutOfMemory3 oom) // Обработка исключения
        {
        ...
        std::cout << "Error code: " << oom.getNum3() << std::endl; // Вывод значения наследуемой переменной-члена
        }
    return 0;
}